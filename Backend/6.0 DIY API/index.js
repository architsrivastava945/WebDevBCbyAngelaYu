...
app.get("/filter", (req, res) => {
  ...
  if(Jokes.length === 0){
    res.status(400).json("No Jokes of this type");
  }else{
    res.json(Jokes);
  }
})
...
app.post("/jokes", (req, res)=>{
  ...
  console.log(jokes[jokes.length-1].id + 1);
  const joke = {
...
app.put("/jokes/:id", (req, res)=>{
  const id = parseInt(req.params.id);
  const joke1 = jokes.findIndex((joke)=>joke.id === id);
  jokes[joke1].jokeText = req.body.text;
  jokes[joke1].jokeType = req.body.type;
  res.send("updated");
})
...
app.patch("/jokes/:id", (req,res)=>{
  ...
  jokes[index].jokeText = req.body.text || jokes[index].jokeText;
  jokes[index].jokeType = req.body.type || jokes[index].jokeType;
...
app.delete("/jokes/all",(req,res)=>{
  ...
  if( masterKey === token){
    jokes = [];
    res.status(200).send("emptied");
  }else{
    res.status(400).send("key does not match");
  }
})
